...
export function Layout() {
  ...
  return (
    <div className="min-h-screen bg-[#EDF2F7] flex flex-col justify-between text-slate-800 font-sans selection:bg-[#002B7F] selection:text-white">
      <OfflineIndicator />
      ...
      <header className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-3 sm:pt-4 md:pt-6 pb-2">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <Link
            to="/profile"
            className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full bg-white border border-slate-200/90 shadow-2xs hover:border-[#002B7F] hover:shadow-md transition-all flex items-center justify-center text-[#002B7F] font-bold text-xs sm:text-sm md:text-base shrink-0 group select-none"
          ...
          <div className="flex-1 flex justify-center px-2">
            ...
            >
              <img
                ...
                className="h-10 sm:h-12 md:h-14 w-auto max-w-[150px] sm:max-w-[200px] object-contain"
                onError={(e) => {
          ...
          <Link
            to="/settings"
            className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full bg-white border border-slate-200/90 shadow-2xs hover:border-[#002B7F] hover:shadow-md transition-all flex items-center justify-center text-slate-700 hover:text-[#002B7F] shrink-0 group select-none"
            title="Настройки портала и навигации"
          >
        ...
        <div className="flex items-center justify-between border-t border-slate-200/60 pt-2 sm:pt-2.5 mt-2 sm:mt-3">
          {/* Left: Current Date */}
          <div className="text-xs sm:text-sm font-semibold tracking-wider text-slate-600 uppercase select-none">
            {formattedDate}
          </div>

          {/* Right: Digital Clock */}
          <div className="text-xs sm:text-sm md:text-base font-mono font-medium tracking-tight text-slate-700 select-none text-right">
            {formattedTime}
          </div>
        </div>
      </header>
      ...
      <footer className="w-full max-w-6xl lg:max-w-7xl 2xl:max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8 pb-4 md:pb-6 pt-2 sticky bottom-0 z-30">
        ...
        <div className="hidden lg:flex items-center justify-between bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] border border-slate-200/80 px-6 py-3">
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className={cn(
                ...
                isHomeActive
                  ? 'bg-[#D6E6F9] text-[#002B7F] shadow-xs'
          ...
          <div>
            <button
              onClick={logout}
              className="flex items-center justify-center px-6 py-2.5 rounded-xl border-2 border-[#E53935] text-[#E53935] hover:bg-red-50 active:bg-red-100 text-sm font-bold transition-colors cursor-pointer"
            >
              <span>ВЫХОД</span>
            </button>
          </div>
        ...
        <div className="lg:hidden flex items-center justify-between bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] border border-slate-100 px-3 sm:px-6 py-2">
          {/* Permanent Home */}
          <Link
            to="/"
            className={cn(
              'flex flex-col items-center justify-center py-1 px-3 rounded-xl transition-all',
              isHomeActive ? 'text-[#002B7F] font-bold' : 'text-slate-600 font-medium'
            )}
          >
          ...
          <Link
            ...
            className={cn(
              'flex flex-col items-center justify-center py-1 px-3 rounded-xl transition-all',
              isSlot1Active ? 'text-[#002B7F] font-bold' : 'text-slate-600 font-medium'
            )}
          >
            {getNavIcon(
              item1.iconName,
          ...
          <Link
            ...
            className={cn(
              'flex flex-col items-center justify-center py-1 px-3 rounded-xl transition-all',
              isSlot2Active ? 'text-[#002B7F] font-bold' : 'text-slate-600 font-medium'
            )}
          >
            {getNavIcon(
              item2.iconName,
          ...
          <button
            onClick={logout}
            className="flex items-center justify-center bg-[#002B7F] text-white px-5 py-2.5 rounded-xl text-xs font-bold shadow-xs active:bg-[#001D56] transition-colors cursor-pointer"
          >
            <span>ВЫХОД</span>
          </button>
        </div>
