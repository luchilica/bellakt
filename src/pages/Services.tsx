...
export default function Services() {
  ...
  return (
    <div className="w-full max-w-xl sm:max-w-2xl lg:max-w-none mx-auto space-y-5 sm:space-y-6 my-auto pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Select value={selectedMonth} onValueChange={setSelectedMonth}>
            <SelectTrigger className="rounded-xl border-slate-200 bg-white font-semibold text-xs sm:text-sm h-10 w-[180px]">
          ...
          <Button
            ...
            className="rounded-xl border-slate-200 bg-white text-slate-700 h-10 px-3 text-xs font-semibold"
          >
            <Download className="w-4 h-4 mr-1.5 text-[#002B7F]" />
            Экспорт
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4">
        <Card className="rounded-2xl border border-slate-200 bg-white shadow-xs">
          <CardContent className="p-4 sm:p-5">
            ...
            <div className="text-xl sm:text-2xl font-extrabold text-[#002B7F] mt-1.5">
        ...
        <Card className="rounded-2xl border border-slate-200 bg-white shadow-xs">
          <CardContent className="p-4 sm:p-5">
            ...
            <div className="text-xl sm:text-2xl font-extrabold text-emerald-600 mt-1.5">
              +215.00 руб.
            </div>
            <p className="text-xs text-slate-500 mt-1">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 bg-white p-1 rounded-xl border border-slate-200 shadow-2xs overflow-x-auto">
        <button
          type="button"
      ...
      {activeTab === 'canteen' && (
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            ...
            <span className="text-xs font-bold text-[#002B7F]">
          ...
          <div className="space-y-3">
            {canteenReceipts.map((receipt) => (
              <Card key={receipt.id} className="rounded-2xl border border-slate-200 bg-white shadow-xs">
                <CardContent className="p-4 sm:p-5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3 mb-3">
                    <div className="flex items-center gap-2">
                      <Receipt className="w-4 h-4 text-[#002B7F]" />
                      <span className="font-bold text-sm text-slate-900">{receipt.id}</span>
                      <span className="text-xs text-slate-500">• {receipt.date} в {receipt.time}</span>
                    </div>
                    <div className="font-extrabold text-sm sm:text-base text-[#002B7F]">
                      {receipt.total.toFixed(2)} руб.
                    </div>
                  </div>

                  <div className="space-y-1.5 text-xs sm:text-sm divide-y divide-slate-50">
                    {receipt.items.map((item, idx) => (
                      <div key={idx} className="flex justify-between items-center pt-1.5 first:pt-0 text-slate-700">
                        <span>{item.name} × {item.count}</span>
                        <span className="font-semibold text-slate-900">{item.price.toFixed(2)} руб.</span>
                      </div>
                    ))}
                  </div>
                </CardContent>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {salaryServices.map((srv) => (
              <Card key={srv.id} className="rounded-2xl border border-slate-200 bg-white shadow-xs">
                <CardContent className="p-4 sm:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-start gap-3.5">
                    <div className="w-11 h-11 rounded-xl bg-[#E8F1FC] text-[#0B4DA2] flex items-center justify-center shrink-0 mt-0.5">
                  ...
                  <div className="flex items-center justify-between md:flex-col md:items-end gap-1 shrink-0 border-t md:border-t-0 pt-3 md:pt-0 border-slate-100">
                    ...
                    <span className="text-xs font-bold uppercase bg-slate-100 text-slate-700 px-2.5 py-1 rounded-md mt-1">
                      Удержано из з/п
                    </span>
                  </div>
                </CardContent>
          ...
          <Card className="rounded-2xl border border-slate-200 bg-white shadow-xs">
            <CardContent className="p-5 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#E8F1FC] text-[#0B4DA2] flex items-center justify-center shrink-0">
                  ...
                  <div>
                    ...
                    <div className="text-xs sm:text-sm text-slate-600 font-medium">{telecomData.plan}</div>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-xs text-slate-600 block">Удержание за перерасход:</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs sm:text-sm">
                <div className="bg-slate-50 rounded-xl p-3.5 space-y-1.5 border border-slate-200">
                  <div className="flex justify-between font-semibold text-slate-700">
                    ...
                    <span className="text-emerald-700 font-bold">Лимит соблюден</span>
                  </div>
                ...
                <div className="bg-slate-50 rounded-xl p-3.5 space-y-1.5 border border-slate-200">
                  <div className="flex justify-between font-semibold text-slate-700">
                    ...
                    <span className="text-amber-700 font-bold">Перерасход трафика</span>
                  </div>
                  <div className="text-slate-600">
